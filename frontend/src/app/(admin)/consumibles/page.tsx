...
import { AlertTriangle, Package, Plus, Pencil, Trash2, X, PlusCircle, MinusCircle, Wallet, FileText } from "lucide-react";
...
export default function ConsumiblesPage() {
  ...
  const [form, setForm] = useState({
    id_propiedad: "", nombre: "", stock_actual: 0, stock_minimo: 0, unidad_medida: "unidades", codigo_consumible: "",
  });
  ...
  const totalUnits = useMemo(() => consumibles.reduce((acc, c) => acc + c.stock_actual, 0), [consumibles]);

  const abrirNuevo = () => {
    setForm({ id_propiedad: propiedades[0]?.id_propiedad || "", nombre: "", stock_actual: 0, stock_minimo: 0, unidad_medida: "unidades", codigo_consumible: "" });
    setModal({ open: true });
  };

  const abrirEditar = (c: Consumible) => {
    setForm({ id_propiedad: c.id_propiedad, nombre: c.nombre, stock_actual: c.stock_actual, stock_minimo: c.stock_minimo, unidad_medida: c.unidad_medida, codigo_consumible: c.codigo_consumible });
  ...
  return (
    <>
      <PageHeader
        eyebrow="Inventario"
        title="Consumibles"
        description="Stock de insumos por propiedad con alertas de mínimo y control de movimientos. Gestión completa de reposición y egresos."
        actions={
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard
          title="Productos"
          value={consumibles.length.toString()}
          caption="Items registrados en catálogo"
          icon={Package}
        />
        <StatCard
          title="Unidades"
          value={totalUnits.toString()}
          caption="Suma total del stock actual"
          icon={Wallet}
        />
        <StatCard
          title="Alertas de stock"
          value={criticos.length.toString()}
          caption="Productos iguales o por debajo del mínimo"
          icon={AlertTriangle}
      ...
      {loading ? (
        <div className="flex h-40 items-center justify-center text-sm text-slate-400">Cargando productos...</div>
      ) : consumibles.length === 0 ? (
        <div className="flex h-40 items-center justify-center rounded-[28px] border border-slate-200 bg-white text-sm text-slate-400">Sin productos registrados</div>
      ) : (
        <div className="grid gap-3">
          {consumibles.map((c) => {
            const st = semaforo(c);
            return (
              <div key={c.id_consumible} className={`flex items-center gap-4 rounded-[24px] border bg-white p-5 transition-all hover:shadow-sm ${
                st === "danger" ? "border-l-4 border-l-rose-500 border-slate-200" :
                st === "warning" ? "border-l-4 border-l-amber-500 border-slate-200" : "border-l-4 border-l-emerald-500 border-slate-200"
              }`}>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${
                      ...
                      {st === "danger" ? "Crítico" : st === "warning" ? "Alerta" : "Óptimo"}
                    </span>
                  </div>
                  <p className="mt-1 font-mono text-xs text-slate-400">{c.codigo_consumible}</p>
                  <div className="mt-1.5 flex flex-wrap gap-x-6 gap-y-1 text-sm text-slate-500">
                    <span className="inline-flex items-center gap-1.5">
                      <Package className="h-3.5 w-3.5 text-slate-400" />
                      {getPropName(c.id_propiedad)}
                    </span>
                ...
                <div className="flex shrink-0 gap-1">
                  ...
                  <button onClick={() => abrirEditar(c)}
                    className="rounded-xl bg-slate-50 p-2.5 text-slate-400 transition-colors hover:bg-slate-100">
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button onClick={() => eliminar(c.id_consumible)}
                    className="rounded-xl bg-slate-50 p-2.5 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-sm rounded-[28px] border border-slate-200 bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Movimiento</p>
                <h3 className="mt-1 text-xl font-semibold text-slate-900">Ajustar stock</h3>
              </div>
              <button onClick={() => setAjusteId(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
            </div>
            <p className="mb-4 text-sm text-slate-500">Usá valores positivos para ingresar stock o negativos para egresar.</p>
            <input type="number" value={ajusteCant} onChange={(e) => setAjusteCant(Number(e.target.value))}
              className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-900" placeholder="Ej. 10 o -5" autoFocus />
            <div className="mt-4 flex justify-end gap-3">
              <button onClick={() => setAjusteId(null)} className="rounded-full border border-slate-200 px-6 py-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50">Cancelar</button>
              <button onClick={() => ajustarStock(ajusteId)}
                className="inline-flex items-center gap-1.5 rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-slate-800">
          ...
          <div className="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <h3 className="mt-1 text-xl font-semibold text-slate-900">
                  {modal.edit ? "Actualizar datos del producto" : "Registrar nuevo insumo"}
                </h3>
              </div>
            ...
            <div className="space-y-4">
              ...
              <input type="text" value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })}
                className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-900" placeholder="Nombre del producto" required />
              {modal.edit && form.codigo_consumible && (
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm text-slate-500">
                  Código: <span className="font-mono font-medium text-slate-800">{form.codigo_consumible}</span>
                </div>
              )}
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <p className="mb-1 text-xs text-slate-400">Stock actual</p>
                  <input type="number" min="0" value={form.stock_actual} onChange={(e) => setForm({ ...form, stock_actual: Number(e.target.value) })}
                    className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-900" placeholder="Stock" />
                </div>
                <div>
                  <p className="mb-1 text-xs text-slate-400">Stock mínimo</p>
                  <input type="number" min="0" value={form.stock_minimo} onChange={(e) => setForm({ ...form, stock_minimo: Number(e.target.value) })}
                    className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-slate-900" placeholder="Mín" />
                </div>
                <div>
                  <p className="mb-1 text-xs text-slate-400">Unidad de medida</p>
